...
bool checkValid(unsigned char* hash, unsigned char* target) {
  ...
  memcpy(diff_target, target, 32);  /* was &target — pointer bug fix */
...
/*
 * calculateMiningData — CKB version
 *
 * The pool has already sent pow_hash and target as binary in mining_job
 * (parsed by parse_mining_notify in stratum.cpp).
 * Here we just copy them and decode extranonce1 into nonce_prefix.
 */
miner_data calculateMiningData(mining_subscribe& mWorker, mining_job mJob){

  miner_data mMiner = init_miner_data();

  /* CKB: pow_hash and target are already decoded to binary by stratum.cpp */
  memcpy(mMiner.pow_hash,          mJob.pow_hash, 32);
  memcpy(mMiner.bytearray_target,  mJob.target,   32);

  /*
   * Decode extranonce1 hex string → nonce_prefix (up to 8 bytes).
   * These occupy nonce bytes [8..15] (upper half of the 128-bit nonce).
   */
  const char* en1     = mWorker.extranonce1.c_str();
  size_t      en1_hex = strlen(en1);
  size_t      en1_bytes = en1_hex / 2;
  if (en1_bytes > 8) en1_bytes = 8;
  memset(mMiner.nonce_prefix, 0, 8);
  for (size_t i = 0; i < en1_bytes; ++i) {
      unsigned v;
      if (sscanf(en1 + i * 2, "%02x", &v) == 1)
          mMiner.nonce_prefix[i] = (uint8_t)v;
  }

  Serial.print("    pow_hash:     ");
  for (int i = 0; i < 32; i++) Serial.printf("%02x", mMiner.pow_hash[i]);
  Serial.println();
  Serial.print("    target:       ");
  for (int i = 31; i >= 0; i--) Serial.printf("%02x", mMiner.bytearray_target[i]);
  Serial.println();
  Serial.print("    nonce_prefix: ");
  for (int i = 0; i < 8; i++) Serial.printf("%02x", mMiner.nonce_prefix[i]);
  Serial.println();
